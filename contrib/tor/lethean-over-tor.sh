...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/letheand"
  then
    letheand="$dir/letheand"
    break
  fi
done
if test -z "$letheand"
then
  echo "letheand not found"
  exit 1
fi
echo "Found: $letheand"

TORDIR="$DIR/lethean-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting letheand"
  tail -n 400 "$HOME/Lethean/letheand.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Monero\ \'" | tail -n 20
  exit 1
fi
